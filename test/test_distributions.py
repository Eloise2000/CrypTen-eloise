...
class TestDistributions:
    ...
    def test_uniform(self):
        self._check_distribution(crypten_eloise.rand, 0.5, 0.083333, lb=0, ub=1)

    def test_normal(self):
        self._check_distribution(crypten_eloise.randn, 0, 1)

    def test_bernoulli(self):
        for p in [0.25 * i for i in range(5)]:

            def bernoulli(*size):
                x = crypten_eloise.cryptensor(p * torch.ones(*size))
                return x.bernoulli()
