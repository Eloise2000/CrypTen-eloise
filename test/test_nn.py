...
from crypten_eloise.common.rng import generate_random_ring_element
from crypten_eloise.common.tensor_types import is_float_tensor
from crypten_eloise.config import cfg
from crypten_eloise.encoder import FixedPointEncoder
...
def linear_to_crypten(pytorch_linear):
    ...
    crypten_linear = crypten_eloise.nn.Linear(in_channels, out_channels)
...
class TestNN:
    ...
    def test_from_shares(self):
        ...
        model = crypten_eloise.nn.Linear(input_size, output_size)
    ...
    def test_global_avg_pool_module(self):
        ...
        # create module
        encr_module = crypten_eloise.nn.GlobalAveragePool().encrypt()
        ...
        for i in range(1, 10):
            ...
            encr_input = crypten_eloise.cryptensor(input)
    ...
    def test_dropout_module(self) -> None:
        ...
        for prob in prob_list:
            for compute_gradients in [True, False]:
                ...
                encr_module = crypten_eloise.nn.from_pytorch(module, input)
    ...
    def test_non_pytorch_modules(self):
        ...
        for module_name in module_args.keys():
            ...
            # create encrypted CrypTen module:
            encr_module = getattr(crypten_eloise.nn, module_name)(*module_args[module_name])
            encr_module.encrypt()
            self.assertTrue(encr_module.encrypted, "module not encrypted")
            ...
            if module_name in binary_modules:
                ...
                encr_inputs = [crypten_eloise.cryptensor(input) for input in inputs]
                ...
                if module_name == "Sqrt":  # Sqrt requires positive input
                    inputs = inputs.abs()
                encr_inputs = crypten_eloise.cryptensor(inputs)
            ...
            if module_name in additional_inputs:
                ...
                for add_inp in additional_inputs[module_name]:
                    ...
                    if torch.is_tensor(add_inp):
                        encr_inputs.append(crypten_eloise.cryptensor(add_inp))
                    else:
            ...
            if torch.is_tensor(encr_output):
                ...
                encr_output = crypten_eloise.cryptensor(encr_output)
            ...
            module = getattr(crypten_eloise.nn, module_name).from_onnx(attributes=local_attr)
    ...
    def test_pytorch_modules(self):
        ...
        for module_name, from_pytorch, compute_gradients in itertools.product(
            ...
            if from_pytorch:
                ...
                encr_module = getattr(crypten_eloise.nn, module_name)(*args, **kwargs)
            ...
            for encrypted in [False, True, True, False, True]:
                ...
                for key in ["weight", "bias"]:
                    if hasattr(module, key):  # if PyTorch model has key
                        ...
                        if isinstance(encr_param, crypten_eloise.nn.Parameter):
                            encr_param = encr_param.data
                        if not crypten_eloise.is_encrypted_tensor(encr_param):
                            encr_param = crypten_eloise.cryptensor(encr_param, src=0)
    ...
    def test_conv(self):
        ...
        for dim in range(1, 3):
            for compute_gradients in [True, False]:
                ...
                encr_kernel = crypten_eloise.cryptensor(kernel)
                encr_kernel.requires_grad = compute_gradients

                # create "encrypted" CrypTen module:
                module = crypten_eloise.nn.Conv(stride, padding, dilation, groups)
                module.encrypt()
    ...
    def test_linear(self):
        ...
        for compute_gradients in [True, False]:
            for dim, size in zip(dims, sizes):
                ...
                # create encrypted CrypTen module:
                encr_module = crypten_eloise.nn.Linear(*dim)
    ...
    def test_parameter_module(self):
        ...
        for trainable in [False, True]:
            for ttype in [
                ...
                param = crypten_eloise.nn.Parameter(ttype(reference), trainable=trainable)
                ...
                for value in [param.data, param(None)]:
                    if ttype == crypten_eloise.cryptensor:
                ...
                if ttype != crypten_eloise.cryptensor:
                    ...
                    for value in [param.data, param(None)]:
                        self.assertTrue(crypten_eloise.is_encrypted_tensor(value))
        ...
        with self.assertRaises(AssertionError):
            param = crypten_eloise.nn.Parameter(list(range(5)))
    ...
    def test_inplace_warning(self):
        ...
        for module in modules:
            ...
            with self.assertLogs(logger=logging.getLogger(), level="WARNING") as cm:
                getattr(crypten_eloise.nn, module)(inplace=True)
    ...
    def test_sequential(self):
        ...
        for num_layers in range(1, 6):
            for compute_gradients in [True, False]:
                ...
                crypten_module_list = [
                    crypten_eloise.nn.Linear(num_feat, num_feat - 1) for num_feat in layer_idx
                ]
                ...
                crypten_sequential = crypten_eloise.nn.Sequential(*crypten_module_list)
    ...
    def test_graph(self):
        ...
        for compute_gradients in [True, False]:
            for num_inputs in [1, 2]:
                ...
                encr_input = crypten_eloise.cryptensor(input)
                encr_input2 = crypten_eloise.cryptensor(input2)
                encr_input.requires_grad = compute_gradients
                encr_input2.requires_grad = compute_gradients

                # for two inputs, sum the inputs first:
                if num_inputs == 1:
                    graph = crypten_eloise.nn.Graph("input", "output")
                elif num_inputs == 2:
                    graph = crypten_eloise.nn.Graph(["input1", "input2"], "output")
                    graph.add_module("input", crypten_eloise.nn.Add(), ["input1", "input2"])
                else:
                ...
                graph.add_module("residual", crypten_eloise.nn.Add(), ["input", "linear"])
    ...
    def test_losses(self):
        ...
        encrypted_input = crypten_eloise.cryptensor(input)
        encrypted_target = crypten_eloise.cryptensor(target)
        ...
        for loss_name in losses:
            for skip_forward in [False, True]:
                enc_loss_object = getattr(crypten_eloise.nn, loss_name)(
        ...
        encrypted_input = crypten_eloise.cryptensor(input)
        encrypted_target = crypten_eloise.cryptensor(onehot(target, num_targets=num_targets))
        enc_loss_object = crypten_eloise.nn.CrossEntropyLoss()
        self.assertEqual(
            enc_loss_object.reduction, "mean", "Reduction used is not 'mean'"
        )

        loss = torch.nn.CrossEntropyLoss()(input, target)
        encrypted_loss = crypten_eloise.nn.CrossEntropyLoss()(
    ...
    def test_distances(self):
        ...
        for distance in distances:
            ...
            x1_enc = crypten_eloise.cryptensor(x1, requires_grad=True)
            x2_enc = crypten_eloise.cryptensor(x2, requires_grad=True)

            dist_fn = getattr(torch.nn, distance)()
            enc_dist_fn = getattr(crypten_eloise.nn, distance)()
            ...
            grad_output_enc = crypten_eloise.cryptensor(grad_output)
    ...
    def test_getattr_setattr(self):
        ...
        class ExampleNet(crypten_eloise.nn.Module):
            def __init__(self):
                super(ExampleNet, self).__init__()
                self.fc1 = crypten_eloise.nn.Linear(20, 1)
        ...
        self.assertTrue(
            isinstance(model.fc1, crypten_eloise.nn.Linear), "modules __getattr__ failed"
        )
    ...
    def test_training(self):
        ...
        model = crypten_eloise.nn.Sequential(
            crypten_eloise.nn.Linear(num_inputs, num_intermediate),
            crypten_eloise.nn.ReLU(),
            crypten_eloise.nn.Linear(num_intermediate, num_outputs),
        )
        model.train()
        model.encrypt()
        loss = crypten_eloise.nn.MSELoss()
        ...
        for _ in range(10):
            for compute_gradients in [True, False]:
                ...
                input = crypten_eloise.cryptensor(input)
                target = crypten_eloise.cryptensor(target)
                if compute_gradients:
                    input.requires_grad = True
    ...
    def test_custom_module_training(self):
        ...
        class ExampleNet(crypten_eloise.nn.Module):
            def __init__(self):
                super(ExampleNet, self).__init__()
                self.fc1 = crypten_eloise.nn.Linear(NUM_FEATURES, BATCH_SIZE)
                self.fc2 = crypten_eloise.nn.Linear(BATCH_SIZE, 2)
        ...
        # encrypt training sample:
        x_train = crypten_eloise.cryptensor(x_orig, requires_grad=True)
        y_train = crypten_eloise.cryptensor(y_one_hot)

        for loss_name in ["BCELoss", "CrossEntropyLoss", "MSELoss"]:
            # create loss function
            loss = getattr(crypten_eloise.nn, loss_name)()
    ...
    def test_batchnorm_module(self) -> None:
        ...
        for batchnorm_fn, size in batchnorm_fn_and_size:
            for is_training in (True, False):
                ...
                encrypted_input = crypten_eloise.cryptensor(tensor, requires_grad=True)
                ...
                # create models:
                enc_model = getattr(crypten_eloise.nn.module, batchnorm_fn)(C)
                ...
                stateless_model = crypten_eloise.nn.module.BatchNormalization().encrypt()
                stateless_model.train(is_training)
                encr_output = stateless_model(
                    (
                        encrypted_input,
                        crypten_eloise.cryptensor(params["weight"]),
                        crypten_eloise.cryptensor(params["bias"]),
                        crypten_eloise.cryptensor(orig_buffers["running_mean"]),
                        crypten_eloise.cryptensor(orig_buffers["running_var"]),
                    )
                )
                if not is_training:
    ...
    def test_unencrypted_modules(self) -> None:
        ...
        linear = crypten_eloise.nn.Linear(input_size[1], output_size[1])
        criterion = crypten_eloise.nn.MSELoss()
        ...
        def _run_test(_sample, _target):
            ...
            for idx, param in enumerate(linear.parameters()):
                ...
                if isinstance(diff, crypten_eloise.CrypTensor):
        ...
        for encrypted in [False, True, False, True]:
            ...
            _run_test(crypten_eloise.cryptensor(sample), crypten_eloise.cryptensor(target))
    ...
    def test_state_dict(self) -> None:
        ...
        def _check_equal(t1, t2):
            ...
            if isinstance(t1, crypten_eloise.CrypTensor):
        ...
        for module_name, args in module_args.items():
            for encrypt in [False, True]:
                ...
                module = getattr(crypten_eloise.nn, module_name)(*args)
                if encrypt:
                    module.encrypt()
                ...
                new_module = getattr(crypten_eloise.nn, module_name)(*args)
                if encrypt:
                    with self.assertRaises(AssertionError):
                ...
                if encrypt:
                    f = io.BytesIO()
                    crypten_eloise.save(module.state_dict(), f)
                    f.seek(0)
                    new_module2 = getattr(crypten_eloise.nn, module_name)(*args)
                    new_module2.encrypt()
                    new_module2.load_state_dict(crypten_eloise.load(f))
        ...
        for num_layers in range(1, 6):
            for encrypt in [False, True]:
                ...
                model = crypten_eloise.nn.Sequential(*module_list)
                if encrypt:
                    model.encrypt()
                ...
                new_model = crypten_eloise.nn.Sequential(*module_list)
                if encrypt:
                    with self.assertRaises(AssertionError):
                ...
                if encrypt:
                    f = io.BytesIO()
                    crypten_eloise.save(model.state_dict(), f)
                    f.seek(0)
                    module_list = [
                        crypten_eloise.nn.Linear(num_feat, num_feat - 1)
                        for num_feat in layer_idx
                    ]
                    new_model2 = crypten_eloise.nn.Sequential(*module_list)
                    new_model2.encrypt()
                    new_model2.load_state_dict(crypten_eloise.load(f))
                    _check_state_dict(new_model2, state_dict)

    def test_to(self) -> None:
        """Test Module.to, Module.cpu, and Module.cuda"""
        module_list = [crypten_eloise.nn.Linear(10, 10) for _ in range(3)]
        model = crypten_eloise.nn.Sequential(*module_list)
    ...
    def test_module_dict(self) -> None:
        ...
        module_dict = crypten_eloise.nn.ModuleDict()
        self.assertEqual(len(module_dict), 0, "ModuleDict initialized incorrect size")

        # Test initialization
        module_dict = crypten_eloise.nn.ModuleDict(
            {"conv2d": crypten_eloise.nn.Conv2d(10, 10, 3), "pool": crypten_eloise.nn.MaxPool2d(3)}
        )
        ...
        self.assertTrue(
            isinstance(module_dict["conv2d"], crypten_eloise.nn.Conv2d),
            "ModuleDict init failed",
        )
        self.assertTrue("pool" in module_dict.keys(), "ModuleDict init failed")
        self.assertTrue(
            isinstance(module_dict["pool"], crypten_eloise.nn.MaxPool2d),
            "ModuleDict init failed",
        )

        # Test setitem
        module_dict["conv1d"] = crypten_eloise.nn.Conv1d(5, 5, 3)
        ...
        self.assertTrue(
            isinstance(module_dict["conv1d"], crypten_eloise.nn.Conv1d),
            "ModuleDict setitem failed",
        )

        # Test pop
        conv = module_dict.pop("conv2d")
        self.assertTrue(isinstance(conv, crypten_eloise.nn.Conv2d), "ModuleDict pop failed")
        ...
        module_dict = crypten_eloise.nn.ModuleDict(
            [["relu", crypten_eloise.nn.ReLU()], ["sigmoid", crypten_eloise.nn.Sigmoid()]]
        )
        self.assertEqual(len(module_dict), 2, "ModuleDict initialized incorrect size")
        self.assertTrue("relu" in module_dict.keys(), "ModuleDict init failed")
        self.assertTrue(
            isinstance(module_dict["relu"], crypten_eloise.nn.ReLU), "ModuleDict init failed"
        )
        self.assertTrue("sigmoid" in module_dict.keys(), "ModuleDict init failed")
        self.assertTrue(
            isinstance(module_dict["sigmoid"], crypten_eloise.nn.Sigmoid),
            "ModuleDict init failed",
        )
    ...
    def test_module_list(self) -> None:
        """Test ModuleDict module"""
        module_list = crypten_eloise.nn.ModuleList()
        self.assertEqual(len(module_list), 0, "ModuleList initialized incorrect size")

        # Test initialization
        module_list = crypten_eloise.nn.ModuleList(
            [crypten_eloise.nn.Conv2d(10, 10, 3), crypten_eloise.nn.MaxPool2d(3)]
        )
        self.assertEqual(len(module_list), 2, "ModuleList initialized incorrect size")
        self.assertTrue(
            isinstance(module_list[0], crypten_eloise.nn.Conv2d),
            "ModuleList init failed",
        )
        self.assertTrue(
            isinstance(module_list[1], crypten_eloise.nn.MaxPool2d),
            "ModuleList init failed",
        )

        # Test append
        module_list.append(crypten_eloise.nn.ReLU())
        self.assertEqual(len(module_list), 3, "ModuleList append failed")
        self.assertTrue(
            isinstance(module_list[2], crypten_eloise.nn.ReLU),
            "ModuleList append failed",
        )

        # Test extend
        module_list.extend([crypten_eloise.nn.Linear(10, 5), crypten_eloise.nn.ReLU()])
        msg = "ModuleList append failed"
        self.assertEqual(len(module_list), 5, msg)
        self.assertTrue(isinstance(module_list[3], crypten_eloise.nn.Linear), msg)
        self.assertTrue(isinstance(module_list[4], crypten_eloise.nn.ReLU), msg)

        # Test insert
        module_list.insert(1, crypten_eloise.nn.Sigmoid())
        msg = "ModuleList append failed"
        self.assertEqual(len(module_list), 6, msg)
        self.assertTrue(isinstance(module_list[1], crypten_eloise.nn.Sigmoid), msg)

        # Test __delitem__
        del module_list[1]
        msg = "ModuleList delitem failed"
        self.assertEqual(len(module_list), 5, msg)
        self.assertTrue(isinstance(module_list[1], crypten_eloise.nn.MaxPool2d), msg)

        # Test __delitem__ with slice
        del module_list[1:3]
        msg = "ModuleList delitem failed with slice input"
        self.assertEqual(len(module_list), 3, msg)
        self.assertTrue(isinstance(module_list[0], crypten_eloise.nn.Conv2d), msg)
        self.assertTrue(isinstance(module_list[1], crypten_eloise.nn.Linear), msg)
    ...
    def test_parameter_initializations(self) -> None:
        ...
        for init, size, private in itertools.product(
            ...
            encrypted = crypten_eloise.cryptensor(tensor) if private else tensor.clone()
            ...
            torch.manual_seed(0)
            encrypted_out = getattr(crypten_eloise.nn.init, init)(encrypted, *args)
    ...
    def test_tutorial_modules(self) -> None:
        ...
        for torch_class, input_size in input_sizes.items():
            ...
            crypten_model = crypten_eloise.nn.from_pytorch(torch_model, dummy_input)
            ...
            test_input_encr = crypten_eloise.cryptensor(test_input)
