...
from crypten_eloise.debug import configure_logging, pdb
...
class TestDebug(MultiProcessTestCase):
    ...
    def test_wrap_error_detection(self) -> None:
        ...
        encrypted_tensor = crypten_eloise.cryptensor(0)
    ...
    def test_correctness_validation(self) -> None:
        for grad_enabled in [False, True]:
            crypten_eloise.set_grad_enabled(grad_enabled)
            ...
            encrypted_tensor.add = lambda y: crypten_eloise.cryptensor(0)
            with self.assertRaises(ValueError):
                encrypted_tensor.add(10)

            # Ensure incorrect validation works properly for value
            encrypted_tensor.add = lambda y: crypten_eloise.cryptensor(tensor)
            with self.assertRaises(ValueError):
                encrypted_tensor.add(10)
