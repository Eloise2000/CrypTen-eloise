...
import crypten_eloise.gradients as gradients
import torch
from crypten_eloise.common.tensor_types import is_float_tensor
from crypten_eloise.config import cfg
from crypten_eloise.gradients import AutogradContext, AutogradFunction
...
class TestAutograd:
    ...
    def test_non_differentiable_marking(self):
        ...
        inputs = [crypten_eloise.cryptensor(input) for input in inputs]
        ctx = AutogradContext()
        ...
        input = crypten_eloise.cryptensor(input, requires_grad=True)
        reference = [True, True, False]
        outputs = [None] * 3
    ...
    def test_inplace(self):
        ...
        for requires_grad in [False, True]:
            result = crypten_eloise.cryptensor(reference, requires_grad=requires_grad)
            if requires_grad:
    ...
    def test_detach(self):
        ...
        for func_name in ["detach", "detach_"]:
            ...
            input1 = crypten_eloise.cryptensor(input1, requires_grad=True)
            input2 = crypten_eloise.cryptensor(input2, requires_grad=True)
    ...
    def test_forward_tracking(self):
        ...
        for requires_grad in [True, False]:
            ...
            input = crypten_eloise.cryptensor(input, requires_grad=requires_grad)
    ...
    def test_autograd_accumulation(self):
        ...
        def test_case6(input, encr_input):
            ...
            encr_output = (
                crypten_eloise.cat([encr_intermediate1, encr_intermediate2], dim=0)
                .mul(0.5)
                .sum()
            )
            return output, encr_output
        ...
        def test_case8(input, encr_input):
            ...
            encr_intermediate2 = crypten_eloise.cat([encr_input, encr_intermediate1])
            encr_intermediate3 = encr_intermediate2.pow(2.0)
            encr_output = (
                crypten_eloise.cat([encr_input, encr_intermediate2, encr_intermediate3])
                .add(-1)
                .sum()
            )
        ...
        def test_case9(input, encr_input):
            ...
            encr_intermediate1 = crypten_eloise.cat([encr_input, encr_input])
            encr_intermediate2 = encr_intermediate1.mean(0, keepdim=True)
            encr_output = crypten_eloise.cat([encr_intermediate2, encr_intermediate1]).sum()
        ...
        for idx, test_case in enumerate(test_cases):
            ...
            input = get_random_test_tensor(size=(12, 5), is_float=True)
            input.requires_grad = True
            encr_input = crypten_eloise.cryptensor(input, requires_grad=True)
