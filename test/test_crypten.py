...
class TestCrypten(MultiProcessTestCase):
    ...
    def setUp(self):
        ...
        if self.rank >= 0:
            crypten_eloise.init()
            crypten_eloise.set_default_cryptensor_type("mpc")
    ...
    def test_przs_generators(self):
        ...
        for device in crypten_eloise.generators["prev"].keys():
            t0 = torch.randint(
                -(2**63),
                2**63 - 1,
                (1,),
                device=device,
                generator=crypten_eloise.generators["prev"][device],
            )
            t1 = torch.randint(
                -(2**63),
                2**63 - 1,
                (1,),
                device=device,
                generator=crypten_eloise.generators["next"][device],
            )
            ...
            for rank in range(self.world_size):
                ...
                if self.rank == receiver:
                    ...
                    receiver_value = crypten_eloise.generators["next"][device].initial_seed()
                    self.assertEqual(sender_value, receiver_value)
                elif self.rank == sender:
                    sender_value = crypten_eloise.generators["prev"][device].initial_seed()
    ...
    def test_global_generator(self):
        ...
        for device in crypten_eloise.generators["global"].keys():
            this_generator = crypten_eloise.generators["global"][device].initial_seed()
    ...
    def test_manual_seeds(self):
        ...
        for seed_name in seed_names:
            for device in crypten_eloise.generators[seed_name].keys():
                orig_seeds[seed_name][device] = crypten_eloise.generators[seed_name][
                    device
                ].initial_seed()
        ...
        crypten_eloise.manual_seed(next_seed, local_seed, global_seed)

        # Check that user-generated seeds are not equal to the auto-generated ones
        for seed_name in seed_names:
            for device in crypten_eloise.generators[seed_name].keys():
                self.assertNotEqual(
                    crypten_eloise.generators[seed_name][device].initial_seed(),
    ...
    def test_cat_stack(self):
        ...
        encrypted1 = crypten_eloise.cryptensor(tensor1)
        encrypted2 = crypten_eloise.cryptensor(tensor2)

        for module in [crypten_eloise, torch]:  # torch.cat on CrypTensor runs crypten.cat
            for op in ["cat", "stack"]:
    ...
    def test_print_log(self):
        ...
        crypten_eloise.print("test")
        crypten_eloise.log("test")

    def test_rand(self):
        """Tests uniform random variable generation on [0, 1)"""
        for size in [(10,), (10, 10), (10, 10, 10)]:
            randvec = crypten_eloise.rand(*size)
        ...
        randvec = crypten_eloise.rand(int(1e6)).get_plain_text()
    ...
    def test_bernoulli(self):
        for size in [(10,), (10, 10), (10, 10, 10)]:
            ...
            randvec = crypten_eloise.bernoulli(probs)
            self.assertTrue(randvec.size() == size, "Incorrect size")
            tensor = randvec.get_plain_text()
            self.assertTrue(((tensor == 0) + (tensor == 1)).all(), "Invalid values")

        probs = torch.Tensor(int(1e4)).fill_(0.2)
        randvec = crypten_eloise.bernoulli(probs).get_plain_text()
    ...
    def test_cryptensor_registration(self):
        ...
        self.assertEqual(crypten_eloise.get_default_cryptensor_type(), "mpc")
        with self.assertRaises(ValueError):
            crypten_eloise.set_default_cryptensor_type(cryptensor_name)
        tensor = crypten_eloise.cryptensor(torch.zeros(1, 3))
        self.assertEqual(crypten_eloise.get_cryptensor_type(tensor), "mpc")

        # register new tensor type:
        @crypten_eloise.register_cryptensor(cryptensor_name)
        class MyCrypTensor(crypten_eloise.CrypTensor):
        ...
        self.assertEqual(crypten_eloise.get_default_cryptensor_type(), "mpc")
        crypten_eloise.set_default_cryptensor_type(cryptensor_name)
        self.assertEqual(crypten_eloise.get_default_cryptensor_type(), cryptensor_name)
        tensor = crypten_eloise.cryptensor(torch.zeros(1, 3))
        self.assertTrue(getattr(tensor, "is_custom_type", False))
        self.assertEqual(crypten_eloise.get_cryptensor_type(tensor), cryptensor_name)

    def test_cryptensor_instantiation(self):
        """Tests that CrypTensors cannot be instantiated."""
        tensor = get_random_test_tensor()
        with self.assertRaises(TypeError):
            encrypted_tensor = crypten_eloise.CrypTensor(tensor)
        encrypted_tensor = crypten_eloise.mpc.MPCTensor(tensor)
        self.assertIsInstance(encrypted_tensor, crypten_eloise.CrypTensor)
    ...
    def test_save_load(self):
        ...
        tensor = get_random_test_tensor()
        cryptensor1 = crypten_eloise.cryptensor(tensor)

        for i, save_closure in enumerate(all_save_fns):
            load_closure = all_load_fns[i]
            f = [
                io.BytesIO() for i in range(crypten_eloise.communicator.get().get_world_size())
            ]
            crypten_eloise.save(cryptensor1, f[self.rank], save_closure=save_closure)
            f[self.rank].seek(0)
            cryptensor2 = crypten_eloise.load(f[self.rank], load_closure=load_closure)
    ...
    def test_plaintext_save_load_from_party(self):
        ...
        comm = crypten_eloise.communicator
        ...
        for dimensions in range(1, 5):
            ...
            for i, save_closure in enumerate(all_save_fns):
                ...
                for src in range(comm.get().get_world_size()):
                    crypten_eloise.save_from_party(
                    ...
                    encrypted_load = crypten_eloise.load_from_party(
                    ...
                    with self.assertRaises(TypeError):
                        crypten_eloise.load_from_party(
                            complete_file, src=src, load_closure=(lambda f: None)
                        )

                    # test pre-loaded
                    encrypted_preloaded = crypten_eloise.load_from_party(
    ...
    def test_plaintext_save_load_module_from_party(self) -> None:
        ...
        for model_type in [TestModule, NestedTestModule]:
            ...
            filename = tempfile.NamedTemporaryFile(delete=True).name
            for src in range(comm.get().get_world_size()):
                crypten_eloise.save_from_party(test_model, filename, src=src)

                result = crypten_eloise.load_from_party(filename, src=src)
                if src == rank:
    ...
    def test_where(self) -> None:
        ...
        for size, y_type in itertools.product(sizes, y_types):
            ...
            condition_encrypted = crypten_eloise.cryptensor(condition_tensor)
            condition_bool = condition_tensor.bool()

            reference_out = torch.where(condition_bool, tensor1, tensor2)

            encrypted_out = crypten_eloise.where(
                condition_bool, encrypted_tensor1, encrypted_tensor2
            )

            y_is_private = crypten_eloise.is_encrypted_tensor(tensor2)
            self._check(
                encrypted_out,
                reference_out,
    ...
    def test_is_initialized(self) -> None:
        ...
        self.assertTrue(crypten_eloise.is_initialized())
        self.assertTrue(comm.is_initialized())

        crypten_eloise.uninit()
        self.assertFalse(crypten_eloise.is_initialized())
        self.assertFalse(comm.is_initialized())

        # note that uninit() kills the TTP process, so we need to restart it:
        if self.rank == self.MAIN_PROCESS_RANK and crypten_eloise.mpc.ttp_required():
