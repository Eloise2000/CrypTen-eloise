...
class AutogradCat(AutogradFunction):
    ...
    def forward(ctx, input, dim=0):
        ...
        return crypten_eloise.cat(input, dim=dim)
...
class AutogradStack(AutogradFunction):
    ...
    def forward(ctx, input, dim=0):
        ...
        return crypten_eloise.stack(input, dim=dim)
...
class AutogradDropout(AutogradFunction):
    ...
    def forward(ctx, input, p=0.5, training=True, inplace=False):
        ...
        # training mode:
        generator = crypten_eloise.generators["global"][input.device]
...
class AutogradHardtanh(AutogradFunction):
    ...
    def forward(ctx, input, min_val=-1, max_val=1):
        ...
        intermediate = crypten_eloise.stack([input - min_val, max_val - input]).gt(0)
...
class AutogradReLU6(AutogradFunction):
    ...
    @staticmethod
    def forward(ctx, input):
        intermediate = crypten_eloise.stack([input, 6 - input]).gt(0)
        grad = intermediate.sum(0).sub(1)
        ctx.save_for_backward(grad)
...
class AutogradDiv(AutogradFunction):
    @staticmethod
    def forward(ctx, input, other):
        if crypten_eloise.is_encrypted_tensor(other):
    ...
    def backward(ctx, grad_output):
        ...
        if crypten_eloise.is_encrypted_tensor(saved[1]):
...
class AutogradRDiv(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        if torch.is_tensor(other) or crypten_eloise.is_encrypted_tensor(other):
...
class AutogradCosineSimilarity(AutogradFunction):
    ...
    def forward(ctx, x1, x2, dim=1, eps=None):
        ...
        if not isinstance(x2, crypten_eloise.CrypTensor):
            x2 = x1.new(x2)

        xy = crypten_eloise.stack([x1, x2], dim=0)  # [x, y]
...
class AutogradAvgPool2D(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        if isinstance(grad_output, crypten_eloise.CrypTensor):
...
class AutogradBinaryCrossEntropy(AutogradFunction):
    ...
    def forward(ctx, pred, target, skip_forward=False):
        ...
        log_pos, log_neg = (
            crypten_eloise.stack([pred, 1.0 - pred]).log(input_in_01=True).unbind(dim=0)
        )
    ...
    def backward(ctx, grad_output):
        ...
        rec_pos, rec_neg = (
            crypten_eloise.stack([pred, 1.0 - pred]).reciprocal(input_in_01=True).unbind(dim=0)
        )
...
class AutogradBinaryCrossEntropyWithLogits(AutogradFunction):
    ...
    def forward(ctx, logit, target, skip_forward=False):
        ...
        # Compute full forward pass
        log_pos, log_neg = (
            crypten_eloise.stack([sigmoid_out, 1.0 - sigmoid_out])
...
class AutogradRAPPORLoss(AutogradFunction):
    ...
    def forward(ctx, logit, target, alpha, skip_forward=False):
        ...
        log_pos, log_neg = (
            crypten_eloise.stack([pred_normalized, 1.0 - pred_normalized])
            .log(input_in_01=True)
            .unbind(dim=0)
        )
