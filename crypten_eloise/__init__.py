...
import crypten_eloise.common  # noqa: F401
import crypten_eloise.communicator as comm
import crypten_eloise.config  # noqa: F401
import crypten_eloise.mpc  # noqa: F401
import crypten_eloise.nn  # noqa: F401
import crypten_eloise.optim  # noqa: F401
import torch
...
def init(config_file=None, party_name=None, device=None):
    ...
    comm._init(use_threads=False, init_ttp=crypten_eloise.mpc.ttp_required())
    ...
    if comm.get().get_rank() < comm.get().get_world_size():
        ...
        if crypten_eloise.mpc.ttp_required():
            crypten_eloise.mpc.provider.ttp_provider.TTPClient._init()
    ...
    if encrypted:
        ...
        # Non-source party
            ...
            if model_class is not None:
                crypten_eloise.common.serial.register_safe_class(model_class)
            result = comm.get().broadcast_obj(None, src)
            if isinstance(result, int) and result == -1:
                raise TypeError("Unrecognized load type from src party")

        if torch.is_tensor(result):
            result = crypten_eloise.cryptensor(result, src=src)
...
def trace(tracing=True):
    crypten_eloise.mpc.get_default_provider().trace(tracing=tracing)


def trace_once():
    crypten_eloise.mpc.get_default_provider().trace_once()


def fill_cache():
    crypten_eloise.mpc.get_default_provider().fill_cache()
