...
from crypten_eloise.gradients import _inverse_broadcast
...
class DPSplitModel(nn.Module):
    ...
    def _process_targets(self, targets):
        ...
        if crypten_eloise.is_encrypted_tensor(targets):
            if self.rr_prob is not None:
                flip_mask = crypten_eloise.bernoulli(flip_probs)
            ...
            # Encrypt targets:
            targets_enc = crypten_eloise.cryptensor(targets, src=self.label_src)
    ...
    def compute_loss(self, targets):
        ...
        if self.alpha is not None:
            ...
            self.preds_enc = crypten_eloise.cryptensor(
        ...
        if self.skip_loss_forward:
            ...
            logits_enc = crypten_eloise.cryptensor(self.logits, src=self.feature_src)
            # BCEWithLogitsLoss
    ...
    def _generate_noise_no_src(self, size):
        return crypten_eloise.randn(size) * self.noise_magnitude

    def _generate_noise_from_src(self, size):
        noise = torch.randn(size) * self.noise_magnitude
        noise = crypten_eloise.cryptensor(noise, src=self.noise_src)
        return noise
    ...
    def _compute_param_grads(self, dLdZ, jacobians):
        ...
        jacobian = crypten_eloise.cryptensor(jacobian, src=self.feature_src)
    ...
    def _backward_full_jacobian(self, grad_output=None):
        ...
        if self.alpha is not None:
            ...
            correction_enc = crypten_eloise.cryptensor(correction, src=self.feature_src)
            dLdZ *= correction_enc
    ...
    def _compute_last_layer_grad(self, grad_output=None):
        ...
        last_input_enc = crypten_eloise.cryptensor(self.last_input, src=self.feature_src)
