...
from crypten_eloise.common.functions.pooling import _adaptive_pool2d_helper
    ...
    def _load_from_state_dict_crypten(self, state_dict, prefix, strict):
        ...
        for name, param in local_state.items():
            ...
            if key in state_dict:
                ...
                param_encrypted = isinstance(input_param, crypten_eloise.CrypTensor)
                if param_encrypted:
                    assert (
                        self.encrypted
                ...
                with crypten_eloise.no_grad(), torch.no_grad():
        ...
        with crypten_eloise.no_grad(), torch.no_grad():
            for param in self.parameters():
                if param.grad is None:
                    continue
                if not self.encrypted and isinstance(param.grad, crypten_eloise.CrypTensor):
                    raise RuntimeError(
        ...
        if mode != self.encrypted:
            ...
            for name, buffer in self.named_buffers(recurse=False):
                ...
                if mode and torch.is_tensor(buffer):
                    self.set_buffer(
                        name,
                        crypten_eloise.cryptensor(buffer, **{"src": src}, requires_grad=False),
                    )
                # decrypt buffer if it's a cryptensor
                elif isinstance(buffer, crypten_eloise.CrypTensor):
    ...
    def __getattribute__(self, name):
        ...
        def forward_function(*args, **kwargs):
            ...
            if self.encrypted and not self.SUPPORTS_PLAINTEXT_INPUTS:
                ...
                if any(isinstance(arg, crypten_eloise.CrypTensor) for arg in args):
                    raise RuntimeError(
...
class Parameter(Module):
    ...
    def __init__(self, param, trainable=True):
        ...
        assert torch.is_tensor(param) or crypten_eloise.is_encrypted_tensor(
            param
        ...
        self.encrypted = crypten_eloise.is_encrypted_tensor(param)
...
class Where(Module):
    ...
    def forward(self, x):
        ...
        return crypten_eloise.where(condition, x1, x2)
...
class Concat(Module):
    ...
    def forward(self, input):
        ...
        return crypten_eloise.cat(input, self.dimension)
...
class Gather(Module):
    ...
    def forward(self, input):
        ...
        if crypten_eloise.is_encrypted_tensor(indices):
        ...
        # CrypTensor input
        if crypten_eloise.is_encrypted_tensor(tensor):
...
class BatchNormalization(Module):
    ...
    def _compute_inv_var(self, running_var):
        ...
        if isinstance(running_var, crypten_eloise.CrypTensor):
...
class _BatchNorm(Module):
    ...
    def train(self, mode=True):
        ...
        if self.training:
            self.inv_var = None
        elif isinstance(self.running_var, crypten_eloise.CrypTensor):
