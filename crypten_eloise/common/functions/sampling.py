...
def randn(*sizes, device=None):
    ...
    """
    u = crypten_eloise.rand(*sizes, device=device).flatten()
    odd_numel = u.numel() % 2 == 1
    if odd_numel:
        u = crypten_eloise.cat([u, crypten_eloise.rand((1,), device=device)])
...
def bernoulli(self):
    ...
    input tensor."""
    return self > crypten_eloise.rand(self.size(), device=self.device)
...
def weighted_index(self, dim=None):
    ...
    r = crypten_eloise.rand(max_weight.size(), device=self.device) * max_weight
