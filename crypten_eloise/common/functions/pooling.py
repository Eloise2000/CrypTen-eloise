...
def _adaptive_pool2d_helper(input, output_size, reduction="mean"):
    ...
    def repeat_row(tensor, dim, ind):
        ...
        return crypten_eloise.cat([x, repeated_row, y], dim=dim)
    ...
    def extend_row(tensor, dim, start_ind, end_ind):
        ...
        if start_ind == 0:
            return crypten_eloise.cat([extended_value, tensor], dim=dim)
        ...
        return crypten_eloise.cat([x, extended_value, y], dim=dim)
