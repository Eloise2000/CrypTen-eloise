...
def _argmax_helper_pairwise(enc_tensor, dim=None):
    ...
    b = crypten_eloise.stack([enc_tensor.roll(i + 1, dims=dim) for i in range(row_length - 1)])
...
def _compute_pairwise_comparisons_for_steps(input_tensor, dim, steps):
    ...
    for _ in range(steps):
        ...
        pairwise_max = crypten_eloise.where(x >= y, x, y)
        enc_tensor_reduced = crypten_eloise.cat([pairwise_max, remainder], dim=dim)
...
def _max_helper_double_log_recursive(enc_tensor, dim):
    ...
    if n == 1:
        ...
        full_max_tensor = crypten_eloise.cat([enc_max_tensor, remainder], dim=dim)
