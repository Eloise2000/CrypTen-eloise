...
class TupleProvider:
    ...
    def _save_requests(self, filepath=None):
        ...
        if len(self.request_cache) == 0:
            crypten_eloise.log("Request cache not saved - cache is empty")
            return
    ...
    def _load_requests(self, filepath=None):
        ...
        if os.path.exists(filepath):
            ...
            crypten_eloise.log(f"Cache requests not loaded - File `{filepath}` not found")

    def _save_tuples(self, filepath=None):
        # TODO: Deal with any overwrite issues
        if len(self.tuple_cache) == 0:
            crypten_eloise.log("Tuple cache not saved - cache is empty")
            return
    ...
    def _load_tuples(self, filepath=None):
        ...
        if os.path.exists(filepath):
            ...
            crypten_eloise.log(f"Tuple cache not loaded - File `{filepath}` not found")
