...
class Model:
    ...
    crypten: crypten_eloise.nn.Module
...
class LogisticRegressionCrypTen(crypten_eloise.nn.Module):
    def __init__(self, n_features=N_FEATURES):
        super().__init__()
        self.linear = crypten_eloise.nn.Linear(n_features, 1)
...
class FeedForwardCrypTen(crypten_eloise.nn.Module):
    def __init__(self, n_features=N_FEATURES):
        super().__init__()
        self.linear1 = crypten_eloise.nn.Linear(n_features, n_features // 2)
        self.linear2 = crypten_eloise.nn.Linear(n_features // 2, n_features // 4)
        self.linear3 = crypten_eloise.nn.Linear(n_features // 4, 1)
...
class ResNetCrypTen(crypten_eloise.nn.Module):
    def __init__(self, n_layers=18):
        super().__init__()
        assert n_layers in [18, 34, 50]
        model = getattr(models, "resnet{}".format(n_layers))(pretrained=True)
        dummy_input = torch.rand([1, 3, 224, 224])
        self.model = crypten_eloise.nn.from_pytorch(model, dummy_input)
