...
def online_learner(
    ...
    crypten_eloise.set_default_backend(backend)
    ...
    for sample in sampler():
        ...
        context = crypten_eloise.cryptensor(sample["context"])
        ...
        if idx == 0:
            ...
            A_inv = crypten_eloise.cat([crypten_eloise.cryptensor(A) for A in A_inv])
            b = crypten_eloise.cryptensor(torch.zeros(num_arms, num_features))
        ...
        reward = crypten_eloise.cryptensor(
        ...
        with crypten_eloise.mpc.ConfigManager("reciprocal_nr_iters", nr_iters):
...
def epsilon_greedy(
    ...
    def score_func(scores, A_inv, b, context):
        explore = crypten_eloise.bernoulli(torch.tensor([epsilon]))
        rand_scores = crypten_eloise.rand(*scores.size())
